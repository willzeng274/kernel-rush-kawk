...
def _split_kernel(
    ...
    for n0 in range(start, end, BLOCK_N):
        ...
        if TREE:
            # A complete prefix tile is visible to every tree row. The scalar
            # branch is uniform within this program; boundary/draft tiles keep
            # the original ancestor and valid-length masks.
            if n0 + BLOCK_N > pos:
                rel = n[None, :] - pos
