"""Two frozen Triton 3.1 SM90 guarded attention compiles; no driver/GPU use."""
import ast
import datetime
import hashlib
...
MANIFEST=Path(__file__).with_name('krxfty_prefix_guard_source.json')
...
def main():
    ...
    (OUT/'krxfty_prefix_guard_source.json').write_text(json.dumps(manifest,indent=2)+'\n')
    results=[]
    for case in manifest['cases']:
        ...
        (OUT/'krxfty_prefix_guard_summary.json').write_text(json.dumps(results,indent=2,default=str)+'\n')
