...
CASES=['qkv_n32_v2_m64_n32_k64_w4_s3']
...
def verify_source():
    ...
    original=HERE/'snapshots/engine/kernels/qkv_n32.py'
    extracted=HERE/'qkv_device.py'
    assert sha(original.read_bytes())==m['original_kernel_sha256']=='9fd1519c9a265e743629f740a8a9fe04f913e2b651019d345f7deba5e4212a50'
    assert sha(extracted.read_bytes())==m['kernel_sha256']=='56d769ed8179200e0821fa29f1b0acd6a01c38822d3d7e4595f3f4dc6e75a828'
    def spans(path):
        source=path.read_text();lines=source.splitlines(keepends=True)
        return {n.name:''.join(lines[min(d.lineno for d in n.decorator_list)-1:n.end_lineno]) for n in ast.parse(source).body if isinstance(n,ast.FunctionDef) and n.name == '_qkv_n32_kernel'}
    assert spans(original)==spans(extracted) and len(spans(extracted))==1
...
def native():
    ...
    spec=importlib.util.spec_from_file_location('frozen_qkv_single',HERE/'qkv_device.py')
    module=importlib.util.module_from_spec(spec);sys.modules[spec.name]=module;spec.loader.exec_module(module)
    kernel=module._qkv_n32_kernel
    ...
    assert constants==dict(BLOCK_M=64,BLOCK_N=32,BLOCK_K=64)
    key='_qkv_n32_kernel'+''.join(sigspec)+str((constvals,extra))
    ...
    record=dict(case=CASES[0],kernel='_qkv_n32_kernel',native_key_sha256=sha(key.encode()),signature=signature,constants=constants,attrs=dict(divisible_by_16=sorted(attrs.divisible_by_16),equal_to_1=sorted(attrs.equal_to_1)),options_explicit=explicit,parsed_options=normalized)
