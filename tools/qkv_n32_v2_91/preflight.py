...
from compile_qkv_n32 import HERE,verify_source,sha
from check_timeout import check

manifest=verify_source()
expected=json.loads((HERE/'NATIVE_EXPECTED.json').read_text())
assert len(expected['launches'])==1 and expected['compile_calls']==0
tracked=None
...
result=dict(status='PASS',source_set_sha256=manifest['source_set_sha256'],engine_files=23,compile_calls_planned=1,
