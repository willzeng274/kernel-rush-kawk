...
def main():
    ...
    source = ROOT / 'engine' / 'sglang_inline_gemv_gu2.py'
    expected = '3e30fbe5130e5b26e236f790e20f9029a7ec912a3cb194f4554f61adcc649f54'
    assert hashlib.sha256(source.read_bytes()).hexdigest() == expected
    spec = importlib.util.spec_from_file_location('sglang_inline_gemv_gu2', source)
    ...
    for name, (n, k) in module.CONFIGS.items():
        ...
        (OUT / f'sglang_gu2_{name}.ptx.inc').write_text(assembly)
        config = dict(id=f'sglang_gu2_{name}', source=source.name,
                      kernel='_sglang_inline_gemv_gu2', cap=544, width=1,
        ...
        (OUT / 'sglang_gu2_summary.json').write_text(json.dumps(results, indent=2, default=str))
