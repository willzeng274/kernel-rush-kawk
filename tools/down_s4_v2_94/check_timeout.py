...
spec=importlib.util.spec_from_file_location('bounded_down_driver',HERE/'compile_down_s4.py')
...
def check():
    ...
    source=(HERE/'compile_down_s4.py').read_text()
    assert source.count('start_new_session=True')==1
    assert runner.CASES==['down_m64_n64_k64_split4_w4_s3','sum_mn163840_split4_block1024']
    assert "==2\n    write('COMPLETION.json'" in source
    assert source.count('triton.compile(')==1
    assert 'signal.setitimer(signal.ITIMER_REAL, 88)' in source
    assert 'remaining=88-' in source and 'min(40,remaining)' in source
    assert 'No retries or resumed builds' in source
    assert not {'torch','triton'} & set(sys.modules)
    checks.append(dict(name='two_sequential40s_cases_88s_internal_cleanup_no_nested_sessions_or_compiler_import',status='PASS'))
