...
for call in calls:
    kfn=next(n for n in ast.parse((HERE/('tree_device.py' if call['compile_requested'] else 'reference_reduce_device.py')).read_text()).body if isinstance(n,ast.FunctionDef) and n.name==call['kernel'])
...
result=dict(method='Execute exact frozen DecodeAttention construction and complete TreeFusedAttention wrapper against CPU tensor/launch stand-ins, then pinned Triton 3.1 create_binder/binder/_get_config and CUDAOptions/parse_options from primary AST. No compiler/Torch/GPU import. Native driver must match every launch field before any build.',
 jit_source_sha256=sha(jit.read_bytes()),backend_source_sha256=sha(compiler.read_bytes()),kernel_source_sha256=sha((HERE/'tree_device.py').read_bytes()),
 wrapper_source_sha256=sha((HERE/'snapshots/preparation/tree_fused_attention.py').read_bytes()),launches=records,wrapper_launch_receipt=launch_receipt(calls),compile_calls=0,gpu_execution=False)
(HERE/'NATIVE_EXPECTED.json').write_text(json.dumps(result,indent=2)+'\n')
print(json.dumps(dict(local_native_preflight='PASS',launches=6,compile_cases_planned=3,compiles=0,keys={r['case']:r['native_key_sha256'] for r in records})))
