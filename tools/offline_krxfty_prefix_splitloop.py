"""Five frozen Triton 3.1 SM90 split-loop attention compiles; no driver/GPU use."""
import ast
import datetime
import hashlib
...
MANIFEST=Path(__file__).with_name('krxfty_prefix_splitloop_source.json')
...
def verify_sources(manifest):
    for source in manifest['sources'].values():
        ...
        for function in source['functions']:
            helper=function_source(snapshot.decode(),function['name'])
            assert helper==function_source(module.decode(),function['name'])
            assert sha(helper.encode())==function['decorated_sha256']
...
def resources(log):
    ...
    return dict(registers_per_thread=int(regs[1]),static_shared_bytes_reported=int(shared[1]) if shared else 0,
...
def child(case):
    ...
    result['launch_shared_bytes']=result['metadata']['shared']
...
def main():
    ...
    (OUT/'krxfty_prefix_splitloop_source.json').write_text(json.dumps(manifest,indent=2)+'\n')
    results=[]
    for case in manifest['cases']:
        ...
        (OUT/'krxfty_prefix_splitloop_summary.json').write_text(json.dumps(results,indent=2,default=str)+'\n')
