...
class VerifierTop8:
    ...
    def _choose(self, table, tokens, logits):
        ...
        self.trial_inputs = (table, tokens, logits)
        try:
            self._check_deadline(deadline)
            snapshot = logits.clone()
            self.trial_inputs = (table, tokens, logits, snapshot)
            ...
            # include top-k, index conversion where required, and index_copy_.
    ...
    def update(self, table, tokens, logits):
        if self.failed:
            raise RuntimeError("top8 picker cannot be reused after a failed trial")
        if not self.chosen:
            self._choose(table, tokens, logits)
        if self.enabled:
            self._candidate(table, tokens, logits)
        else:
            self._original(table, tokens, logits)
