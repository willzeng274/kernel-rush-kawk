...
class Recycler:
    ...
    def update_verifier(self, tokens: torch.Tensor, logits: torch.Tensor) -> None:
        """Explicit verifier path; prompt seeding always uses ``update``."""
        if self.verifier_top8 is None:
            self.update(tokens, logits)
        else:
            self.verifier_top8.update(self.table, tokens, logits)
