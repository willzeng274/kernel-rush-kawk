...
spec=importlib.util.spec_from_file_location('bounded_tree_driver',HERE/'compile_tree_fusion.py')
...
def check():
    ...
    source=(HERE/'compile_tree_fusion.py').read_text()
    assert source.count('start_new_session=True')==1
    assert runner.CASES==['gp16_basic','gp32_large','gp32_small_tile']
    ...
    assert 'remaining=88-' in source and 'min(25,remaining)' in source
    assert 'No retries or resumed builds' in source
    assert not {'torch','triton'} & set(sys.modules)
    checks.append(dict(name='three_sequential25s_cases_88s_internal_cleanup_no_nested_sessions_or_compiler_import',status='PASS'))
