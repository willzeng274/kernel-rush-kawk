...
from compile_tree_fusion import HERE,verify_source,sha
from check_timeout import check

manifest=verify_source()
expected=json.loads((HERE/'NATIVE_EXPECTED.json').read_text())
assert len(expected['launches'])==6 and expected['compile_calls']==0
...
result=dict(status='PASS',source_set_sha256=manifest['source_set_sha256'],engine_files=21,compile_calls_planned=3,
