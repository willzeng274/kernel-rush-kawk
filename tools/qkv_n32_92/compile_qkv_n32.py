"""One proposed source-exact SM90 build; requires separate root authorization."""
...
def verify_source():
    ...
    assert len(m['engine_files'])==22 and m['cases']==CASES
    assert m['bounds']==dict(single_worker=True,total_seconds=90,case_seconds=40,retry=False,expand_search=False)
    assert sha((HERE/'snapshots/preparation/SOURCE_SHA256.txt').read_bytes())==m['source_set_sha256']
    original=HERE/'snapshots/engine/kernels/gemm.py'
    extracted=HERE/'gemm_device.py'
    assert sha(original.read_bytes())==m['original_gemm_sha256']=='89bb37e315fa66694c6451ce5f5926ce9e1918b2a51c4131aae338125e9f3a4d'
    assert sha(extracted.read_bytes())==m['kernel_sha256']=='e4940ce29e52bd3e29123af8aac14094553346ba89dc7203ebfe97a110d5d804'
    def spans(path):
        source=path.read_text();lines=source.splitlines(keepends=True)
        return {n.name:''.join(lines[min(d.lineno for d in n.decorator_list)-1:n.end_lineno]) for n in ast.parse(source).body if isinstance(n,ast.FunctionDef) and n.name in ('_norm_stats','_normed_tile','_skinny_kernel')}
    assert spans(original)==spans(extracted) and len(spans(extracted))==3
...
def native():
    ...
    spec=importlib.util.spec_from_file_location('frozen_qkv_gemm',HERE/'gemm_device.py')
    module=importlib.util.module_from_spec(spec);sys.modules[spec.name]=module;spec.loader.exec_module(module)
    kernel=module._skinny_kernel
    kernel.create_binder()
    values={n:Pointer() for n in ('a_ptr','w_ptr','c_ptr','part_ptr','y_ptr','wn_ptr','xout_ptr')}
    values.update(M=64,N=6144,K=2560,stride_am=2560,stride_wn=2560,k_per_split=2560,num_tiles=192,eps=0.0,BLOCK_M=64,BLOCK_N=32,BLOCK_K=64,SPLIT_K=1,NORM=False)
    explicit=dict(num_warps=4,num_stages=3,debug=kernel.debug)
    ...
    assert constants==dict(BLOCK_M=64,BLOCK_N=32,BLOCK_K=64,SPLIT_K=1,NORM=False)
    key='_skinny_kernel'+''.join(sigspec)+str((constvals,extra))
    ...
    record=dict(case=CASES[0],kernel='_skinny_kernel',native_key_sha256=sha(key.encode()),signature=signature,constants=constants,attrs=dict(divisible_by_16=sorted(attrs.divisible_by_16),equal_to_1=sorted(attrs.equal_to_1)),options_explicit=explicit,parsed_options=normalized)
    assert json.loads(json.dumps([record]))==expected['launches'],'native binder/parsed-options mismatch'
    write('NATIVE_BINDER_ACTUAL.json',dict(launches=[record],jit_source_sha256=expected['jit_source_sha256'],backend_source_sha256=expected['backend_source_sha256'],kernel_source_sha256=expected['kernel_source_sha256'],cuda_execution=False))
    assert 'torch' not in sys.modules
    return {CASES[0]:(kernel,signature,constants,attrs,parsed.__dict__,record)}
...
def child(name):
    ...
    stem=OUT/name
    result=dict(case=name,native=record,parsed_options=options,runtime_grid=[192],runtime_MNK=[64,6144,2560],runtime_strides=[2560,2560,6144],cuda_execution=False,started_at=utc())
    ...
    assert all('m64n32k16' in x for x in inv['wgmma']['forms'])
...
def main():
    ...
    native() # The complete native launch record is checked BEFORE the only build.
    write('NATIVE_PREFLIGHT.json',dict(status='PASS',launches=1,compile_calls_before_preflight=0,source_set_sha256=manifest['source_set_sha256'],expected_sha256=sha((HERE/'NATIVE_EXPECTED.json').read_bytes()),cuda_execution=False))
    results=[]
    for name in CASES:
    ...
    verify_source();assert len(results)==len(list(OUT.glob("*.compile-start.json")))==1
    write('COMPLETION.json',dict(status='PASS',compile_calls=1,total_seconds=time.monotonic()-started,source_verified_before_after=True,cuda_execution=False,completed_at=utc()))
