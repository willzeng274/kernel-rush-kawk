...
kfn=next(n for n in ast.parse((HERE/'gemm_device.py').read_text()).body if isinstance(n,ast.FunctionDef) and n.name=='_skinny_kernel')
...
key='_skinny_kernel'+''.join(spec)+str((const,extra))
record=dict(case='qkv_m64_n32_k64_w4_s3',kernel='_skinny_kernel',native_key_sha256=sha(key.encode()),signature=signature,constants=constants,attrs=dict(divisible_by_16=sorted(attrs.divisible_by_16),equal_to_1=sorted(attrs.equal_to_1)),options_explicit=explicit,parsed_options=parsed)
result=dict(method='Pinned create_binder/binder/_get_config and CUDAOptions/parse_options executed from primary AST without compiler/Torch/GPU import. Native worker must match every field before compiling.',jit_source_sha256=sha(jit.read_bytes()),backend_source_sha256=sha(compiler.read_bytes()),kernel_source_sha256=sha((HERE/'gemm_device.py').read_bytes()),launches=[record],compile_calls=0,gpu_execution=False)
