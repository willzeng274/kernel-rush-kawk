...
def verify_source(manifest):
    ...
    assert sha(json.dumps(files,sort_keys=True).encode())==manifest['source_tree_sha256']
    module=HERE/'accept.py'
    assert sha(module.read_bytes())==manifest['compiler_module_sha256']
    exact=function_source((engine/'kernels/accept.py').read_text(),'accept_kernel')
    assert exact==function_source(module.read_text(),'accept_kernel')
    assert sha(exact.encode())==manifest['decorated_accept_sha256']
    return module
...
def child(case):
    ...
    expected = ['blk_ptr','cand_ptr','child_start_ptr','child_list_ptr','child_par_ptr',
                'masks_ptr','depth_ptr','done_ptr','nseen_ptr','pos_ptr','limit_ptr',
                'root_ptr','path_idx_ptr','path_len_ptr','acc_tok_ptr','acc_cnt_ptr',
                'CAP','R','C','P','MAXA','GUARD']
    pointer_count=16
    wide_pointers={0,1,5,11,14}
    options={'num_warps':1}
...
def main():
    ...
    for case in manifest['cases']:
        ...
        try:
            ...
            (OUT / (case['id'] + '.compiler.stdout')).write_text(run.stdout)
            (OUT / (case['id'] + '.compiler.stderr')).write_text(run.stderr)
    ...
    if len(results) != 6 or any(not r.get('success') for r in results):
        raise SystemExit(1)
