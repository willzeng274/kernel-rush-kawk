...
def main():
    ...
    record = json.loads(Path(__file__).with_name('public_prefill_source.json').read_text())
    source = ROOT / 'engine/public_prefill_kernel.py'
    assert hashlib.sha256(source.read_bytes()).hexdigest() == record['compiler_module_sha256']
    spec = importlib.util.spec_from_file_location('public_prefill_kernel_exact', source)
    ...
    for name, length in [('generic_s', 130), ('divisible16_s', 512), ('folded_s1', 1)]:
        ...
        (OUT / 'public_prefill_summary.json').write_text(json.dumps(results, indent=2, default=str) + '\n')
