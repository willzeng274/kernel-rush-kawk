...
def _load_tma_bf16_planes(LO_DESC, HI_DESC, n0, k0, TN: tl.constexpr, BK: tl.constexpr):
    lo = tl._experimental_descriptor_load(LO_DESC, [n0, k0], [TN, BK], tl.uint8).to(tl.uint16)
    hi = tl._experimental_descriptor_load(HI_DESC, [n0, k0], [TN, BK], tl.uint8).to(tl.uint16)
    return (lo | (hi << 8)).to(tl.uint16).to(tl.bfloat16, bitcast=True)
