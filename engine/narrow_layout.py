"""Fixed original-weight narrow MMA bindings and evidence from actual graph capture."""
from dataclasses import dataclass
import torch
from narrow_config import SHAPES as CONFIGS, narrow_config
from narrow_bf16 import narrow_projection

SOURCE = '5906670d360d3856d05ad960ecd5bbbbfd12d1655f882d2cc09b9ae0b69cab67'
FAMILIES = ('qkv', 'output', 'gateup', 'down')
...
def family_tensors(e, name):
    ...
    if name == 'gateup':
        return e.normalized, e.gateup, [p[1] for p in e.packed]
    if name == 'down':
        return e.intermediate, e.branch, [l.mlp.down_proj.weight for l in e.layers]
    raise ValueError('unsupported narrow MMA family')


def validate_binding(name, x, weight, out):
    n, k = CONFIGS[name]
    rows = x.shape[0]
    narrow_config(rows, name)
    if (tuple(x.shape) != (rows, k) or tuple(weight.shape) != (n, k)
            or tuple(out.shape) != (rows, n)):
        raise ValueError('unsupported narrow MMA tensor shape')
    for t in (x, weight, out):
        if (not t.is_cuda or t.dtype != torch.bfloat16 or t.device != x.device
            ...
            raise ValueError('narrow MMA requires aligned contiguous BF16 CUDA tensors')
    regions = sorted(interval(t) for t in (x, weight, out))
    if any(a[1] > b[0] for a, b in zip(regions, regions[1:])):
        raise ValueError('narrow MMA tensor storage overlaps')
...
class NarrowLayout:
    ...
    def __init__(self, e, native, families, private=None):
        ...
        if not self.families or set(families) != set(self.families):
            raise ValueError('invalid selected narrow MMA families')
        self.bindings = {}
        self.rows = e.batch
        self.configs = {name: narrow_config(self.rows, name) for name in self.families}
        self.observer = None
        for name in self.families:
            x, out, weights = family_tensors(e, name)
            if len(weights) != 36:
                raise ValueError('expected all 36 narrow MMA layer weights')
    ...
    def fingerprint(self):
        if self.configs != {name: narrow_config(self.rows, name) for name in self.families}:
            raise RuntimeError('prepared narrow MMA specialization changed')
        result = [('configuration', self.rows, tuple(
            (name, tuple(sorted(cfg['constants'].items())), cfg['warps'],
             cfg['stages'], cfg['fusion'], cfg['grid'])
            for name, cfg in self.configs.items()))]
        for key, (x, w, y, sig) in self.bindings.items():
            current = (signature(x), signature(w), signature(y))
            if current != sig:
                raise RuntimeError('prepared narrow MMA tensor storage changed')
            result.append((key, sig))
        return tuple(result)

    def begin_capture(self, graph, steps):
        if self.observer is not None or steps < 1:
            raise RuntimeError('nested or invalid narrow MMA capture')
        self.fingerprint()
    ...
    def finish_capture(self, graph):
        ...
        if observed is None or observed[0] is not graph:
            raise RuntimeError('foreign narrow MMA capture graph')
    ...
    def run(self, name, layer, x, weight, out):
        if name not in self.families or x.shape[0] != self.rows:
        ...
        if binding is None or (x is not binding[0] or weight is not binding[1]
            ...
            raise RuntimeError('foreign selected narrow MMA binding')
        if (signature(x), signature(weight), signature(out)) != binding[3]:
            raise RuntimeError('stale selected narrow MMA binding')
        observed = self.observer
        if observed is not None and not torch.cuda.is_current_stream_capturing():
            raise RuntimeError('narrow MMA evidence requires active CUDA capture')
        cfg = self.configs[name]
        narrow_projection[cfg["grid"]](x, weight, out, **cfg["constants"],
            num_warps=cfg["warps"], num_stages=cfg["stages"], enable_fp_fusion=cfg["fusion"])
