...
""".replace("SHARED_BYTES", str(2 * k)).replace("K_SIZE", str(k))
    # Match the CUDA source's two X preloads, then two W preloads, before
    # either dot8. Separate packed registers keep both vectors live.
    loads = []
    for unroll in range(2):
        registers = ",".join(f"x{i}" for i in range(4 * unroll, 4 * unroll + 4))
        loads.append(f"""
...
""")
    for unroll in range(2):
        registers = ",".join(f"w{i}" for i in range(4 * unroll, 4 * unroll + 4))
        loads.append(f"""
 add.u32 koff, ki, {8 * unroll};
 mul.lo.u32 koff, koff, 2;
...
"""
    ...
    return start + "".join(loads) + "".join(chunks) + finish
