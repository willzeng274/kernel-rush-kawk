...
def _load_tma_bf16_planes(LO_DESC, HI_DESC, n0, k0, TN: tl.constexpr, BK: tl.constexpr):
    # Descriptor domain [N,K//4], UINT32, original K-byte row stride.
    # Each loaded word contains four consecutive original uint8 plane bytes.
    ...
    low_bytes = (lo[:, :, None] >> shift[None, None, :]) & 255
    high_bytes = (hi[:, :, None] >> shift[None, None, :]) & 255
    bits = (low_bytes | (high_bytes << 8)).to(tl.uint16)
