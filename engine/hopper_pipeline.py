"""K64 Hopper pipelines only where split ranges match K128 partitioning.
...
class HopperPipelinePlan:
    def __init__(self, x, weight, output, tile_m, splits):
        ...
        if (not 2 <= self.batch <= 32 or self.k not in (2560, 4096, 9728)
                ...
                or not self.same_partition(self.k, splits)):
    ...
    @staticmethod
    def same_partition(k, splits):
        return ((k + splits * 64 - 1) // (splits * 64) * 64
                == (k + splits * 128 - 1) // (splits * 128) * 128)
    ...
    def configurations(n, k, batch, sm_count):
        ...
        return tuple((tile, split) for tile, split
                     in ((128, m128_splits), (64, m64_splits))
                     if HopperPipelinePlan.same_partition(k, split))
...
class HopperPipelineLayout(HopperTilesLayout):
    ...
    def _select(self, name, template, weights, output, generator, deadline):
        if time.monotonic() >= deadline:
            return None
        configurations = HopperPipelinePlan.configurations(
            weights[0].shape[0], template.shape[1], self.batch, self.sm_count)
        ...
        for tile_m, splits in configurations:
            if time.monotonic() >= deadline:
                break
